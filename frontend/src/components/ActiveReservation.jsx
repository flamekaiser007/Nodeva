...
export default function ActiveReservation({ reservation, onSettled }) {
  ...
  async function confirm() {
    ...
    try {
      const status = await confirmAndPay(reservation.reservation_id, {
        description: `Reservation ${reservation.reservation_id.slice(0, 8)}`,
        onError: setError,
      })
      setStatus(status)
    } catch (e) {
      // confirmAndPay already called onError (=setError) for the cases a
      // user can actually do something about (payment failed, dismissed
      // the modal) -- reporting the sentinel itself here would double up
      // the message, so only report what it didn't already show.
